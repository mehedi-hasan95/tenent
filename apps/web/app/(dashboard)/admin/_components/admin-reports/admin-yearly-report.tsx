...
import { adminYearlyReportsAction } from "@/api/reports/admin/admin-report-action"

export const description = "An area chart with a legend"

export const AdminYearlyReport = () => {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ["admin-yearly-report", startMonthParam, endMonthParam],

    queryFn: async () => {
      return adminYearlyReportsAction({
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-start justify-between gap-4">
        <div>
          <CardTitle>Admin Yearly Report</CardTitle>

          <CardDescription>
            {hasRange
              ? "Showing quantity and total sales for the selected range"
              : "Showing the default admin report"}
          </CardDescription>
        </div>
